          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr,index) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" :class="{active:skuSaleAttrValue.isChecked==1}" 
                v-for="(skuSaleAttrValue,index) in spuSaleAttr.spuSaleAttrValueList" 
                :key="skuSaleAttrValue.id"
                @click="changeActive(skuSaleAttrValue,spuSaleAttr.spuSaleAttrValueList)">{{skuSaleAttrValue.saleAttrValueName}}</dd>
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a href="javascript:" class="mins" @click="skuNumber>1?skuNumber--:skuNumber=0">-</a>
              </div>
              <div class="add">
                <!-- 以前的路由跳转从a跳到b路由，在这里，加入购物车跳转，发请求把购买产品信息通过请求的形式通知服务器，服务器进行相应存储 -->
...
<script>
  ...
  export default {
    ...
    methods:{
      changeActive(saleAttrValur,arr){
        //排他思想
        arr.forEach(item => {
          item.isChecked = '0'
        });
        saleAttrValur.isChecked = 1
      },
      //表单元素修改产品个数
     ...
     async addShopcar(){
        //发请求===将产品加入到数据库（通知服务器）
      // try ，catch如果成功了执行try失败了执行catch
      try {
        // 成功
         await this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuid,skuNum:this.skuNumber})
        //路由跳转//将产品信息带给下一级路由组件
        //产品信息的数据比较复杂，通过会话存储
